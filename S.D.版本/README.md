# AI 中小微企业智能财务系统 · Phase 1（S.D.版本 v0.6）

> 依据《企划书》（基础方针）与 2026-09-07 会议《Phase 1 开发计划》（落地方案）实现。
> 一句话目标：**做出可用的「壳子 + 全链路」——把票据照片变成三大报表**。
> v0.6：严格对照 ios-visual-design skill 全库（13 篇 HIG 参考）大改（`documents/designs/hig-overhaul-v0.6.md`）。

## v0.6 新增（HIG 全库大改）

- **Liquid Glass 导航层**：侧边栏/移动端底栏改为玻璃材质（blur 28px + saturate），主内容占满全宽、**滚动时从玻璃导航下方透视穿过**；`prefers-reduced-transparency` 时自动回退实心材质
- **状态系统补全**：空状态组件（无票据/搜索无结果/无报表 → 符号+说明+主操作按钮）；明细与报表加载骨架屏（与最终布局一致的占位）；保存按钮内联 spinner「保存中…」防重复提交；按钮不可用态
- **键盘可达**：全部可点元素 `:focus-visible` 双层焦点环；上传区支持 Tab 聚焦 + Enter/空格触发
- **明细页搜索栏**：占位符标明范围，键入即过滤（号码/科目/备注/类型/销售方/文件名），一键清除，与状态筛选 chips 共存
- **启动状态恢复**：刷新/重开自动回到上次浏览的页面
- **语义化触觉反馈**：上传成功轻振、失败双振（仅支持的移动设备）
- 逐项 skill 出处对照 + 交付前核对清单见 `documents/designs/hig-overhaul-v0.6.md`
